...
static void _render_vertical_time(I8U *string, I8U offset, BOOLEAN b_bold, BOOLEAN b_180_rotation)
{
	I8U *p0, *p1, *p2;
	I8U char_len, i, j;
	I8U buf1[32];
...
static void _fill_vertical_local_clock(BOOLEAN b_180_rotation)
{
	...
	if (b_180_rotation)
		_render_vertical_time(string, 24, TRUE, b_180_rotation);
	else
		_render_vertical_time(string, 64, TRUE, b_180_rotation);
	...
	if (b_180_rotation)
		_render_vertical_time(string, 64, FALSE, b_180_rotation);
	else
		_render_vertical_time(string, 24, FALSE, b_180_rotation);
}
...
static void _render_calendar_weekday_large(SYSTIME_CTX time)
{
	I8U string[64];
	...
	if (cling.ui.fonts_type == LANGUAGE_TYPE_ENGLISH) {	
		...
		sprintf((char *)string, "%s", week_cn_s[time.dow]);
		FONT_load_characters(cling.ui.p_oled_up+256+96, (char *)string, 16, 128, FALSE);
	} else {
		sprintf((char *)string, "%s", week_cn_t[time.dow]);
		FONT_load_characters(cling.ui.p_oled_up+256+96, (char *)string, 16, 128, FALSE);
	}
}

static I16U _render_middle_section_large(I8U len, I8U *string, I8U margin, I8U mode, I16U offset)
{
	I16U i, j;
	I8U *p0, *p1, *p2;
	const I8U *pin;
...
static void _render_icon_switch(I16U offset)
{
	I16U j;
	I8U *p0;
	const I8U *pin;
...
static void _render_vertical_character(I8U *string, I8U offset, I8U margin, I8U len, I8U b_24_size, BOOLEAN b_font_lib)
{
	I8U *p0, *p1, *p2;
...
static void _fill_vertical_skin_temp()
{
  ...
  integer = cling.therm.current_temperature/10;
	...
	string[0] = ICON_MIDDLE_SKIN_TEMP;
	len = 1;
	_render_vertical_character((I8U *)string, 0, margin, len, b_24_size, FALSE);
	...
	string[0] = ICON_MIDDLE_WEATHER_UNIT;
	len = 1;
...
static void _fill_vertical_steps()
{
	...
	Y_SPRINTF("UI: vertical step - %d, %d", cling.ui.vertical_index, stat);
...
static void _fill_vertical_heart_rate()
{
	...
	if (cling.ui.clock_sec_blinking) {
		...
		len = 0;
		string[len++] = ICON_MIDDLE_HEART_RATE;
		_render_vertical_character((I8U *)string, 0, margin, len, b_24_size, FALSE);
	} else {
			cling.ui.clock_sec_blinking = TRUE;
	}
...
static void _middle_row_horizontal(I8U mode)
{
		...
		if (cling.reminder.total>0) { 
			...
			if (cling.reminder.ui_alarm_on) {
				...
				if (cling.reminder.ui_hh >= 24 || cling.reminder.ui_mm >= 60) {
					N_SPRINTF("[UI] alarm invalid - %d:%d", cling.reminder.ui_hh, cling.reminder.ui_mm);
					len = sprintf((char *)string, "--:--");
				} else {
					len = sprintf((char *)string, "%02d:%02d", cling.reminder.ui_hh, cling.reminder.ui_mm);
					
					N_SPRINTF("[UI] ui_hh: %d, ui_mm: %d", cling.reminder.ui_hh, cling.reminder.ui_mm);
				}
			}
		} else {
  		  len = 0;
	...
	if (len > 0) {
		offset = _render_middle_section_large(len, string, margin, mode, offset);
	}
	...
	if (mode == UI_MIDDLE_MODE_CLOCK) {
			_render_calendar_weekday_large(cling.time.local);
	} else if (mode == UI_MIDDLE_MODE_DISTANCE) {
		if (cling.ui.fonts_type == LANGUAGE_TYPE_ENGLISH) {	
...
static void _top_icon_horizontal_render(I8U mode)
{
	I16U i;
	I16U offset = 0; // Pixel offet at top row
	I8U string[64];
	...
	if (mode == UI_TOP_MODE_NONE) {
		...
		_render_one_icon(ICON_TOP_OK_LEN, cling.ui.p_oled_up+108, asset_content+ICON_TOP_OK);
	} else if (mode == UI_TOP_MODE_WORKOUT_START) {
		_render_one_icon(ICON_TOP_WORKOUT_START_LEN, cling.ui.p_oled_up+108, asset_content+ICON_TOP_WORKOUT_START);
	} else if (mode == UI_TOP_MODE_WORKOUT_STOP) {
		_render_one_icon(ICON_TOP_WORKOUT_STOP_LEN, cling.ui.p_oled_up+108, asset_content+ICON_TOP_WORKOUT_STOP);
	}
...
static void _top_icon_vertical_270_render(I8U mode)
{
	if (mode == UI_TOP_MODE_NONE) {
		return;
	} else if (mode == UI_TOP_MODE_STEPS) {
		...
		_top_icon_horizontal_render(mode);
	}
}

static void _top_icon_render(I8U mode, BOOLEAN b_force_horizontal)
{
	if ((cling.ui.clock_orientation == 1) || b_force_horizontal) {
		_top_icon_horizontal_render(mode);
	} else {
		_top_icon_vertical_270_render(mode);
	}
}
...
static void _render_calendar(I8U *buf, SYSTIME_CTX time)
{
	...
	len = sprintf((char *)string, "%d/%02d", time.month, time.day);
	FONT_load_characters(buf+(128-len*6), (char *)string, 8, 128, FALSE);
}

static void _render_calendar_rotation(SYSTIME_CTX time, I8U offset, BOOLEAN b_90_degree)
{
	I8U data_buf[128];
	I8U string[32];
	I8U line_len;
...
static void _core_display_horizontal_carousel(I8U left, I8U top, I8U right, BOOLEAN b_render)
{
	I8U start_offset;
	I8U char_len;
...
static void _core_display_vertical_carousel(I8U left, I8U top, I8U right, BOOLEAN b_render)
{
	I8U string[32];
	I8U len, margin;
	I8U b_24_size = 24;
	...
	string[0] = left;
	len = 1;
	_render_vertical_character((I8U *)string, 9, margin, len, b_24_size, FALSE);
	
	string[0] = top;
	len = 1;
	_render_vertical_character((I8U *)string, (9+42), margin, len, b_24_size, FALSE);
	
	string[0] = right;
	len = 1;
	_render_vertical_character((I8U *)string, (9+84), margin, len, b_24_size, FALSE);
...
static void _core_display_carousel(I8U left, I8U top, I8U right, BOOLEAN b_render)
{
	...
	if (cling.ui.clock_orientation == 1) {
		_core_display_horizontal_carousel(left, top, right, b_render);
	} else {
		_core_display_vertical_carousel(left, top, right, b_render);
	}
}
...
static void _display_frame_workout(I8U index, BOOLEAN b_render)
{
	...
	_top_icon_render(UI_TOP_MODE_RETURN, TRUE);
	_top_icon_render(UI_TOP_MODE_OK, TRUE);
...
static void _display_stopwatch_core(I8U *string1, I8U len1, I8U *string2, I8U len2, I8U mode, BOOLEAN b_center)
{
	...
	for (i = 0; i < len1; i++) {
		...
		if (mode == UI_TOP_MODE_HEART_RATE) {
			if ((string1[i] >= '0') && (string1[i] <= '9')) {
				pin = (I8U *)(asset_content+asset_pos[512+string1[i]+152]);
				char_len = asset_len[512+string1[i]+152];
			} else {
...
void UI_state_machine()
{
	...
	switch (u->state) {
		...
		{
			if (u->state_init) {
				N_SPRINTF("[UI] low power (or charging) shown, %d, %d", u->frame_index, u->frame_cached_index);
				u->state_init = FALSE;
		...
		case UI_STATE_DARK:
		{
			if (t_curr > (u->display_to_base+u->frame_interval)) {
				...
				*/
				...
				N_SPRINTF("[UI] screen go dark - %d, %d", u->frame_index, u->frame_cached_index);
			}
			break;
		}
		default:
