...
static uint8_t current_system_power_mode = POWER_MODE_DEEP_SLEEP;

void pm_stay_alive()
{
	current_system_power_mode = POWER_MODE_WAKE_UP;
}

void pm_resume_sleep()
{
	current_system_power_mode = POWER_MODE_DEEP_SLEEP;
}
...
static void pm_init_wake_up(void)
{
    ...
    adapter_call_backs_t *p_Ad;
		...
		if(current_system_power_mode == POWER_MODE_WAKE_UP){
			return ;
		}
    // Inform Adapters
