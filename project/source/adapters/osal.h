...
#define  ASSERT_ERROR      OS_ASSERT 
#define  ASSERT_WARNING    OS_ASSERT  
/**
 * \brief OS Notification action
 */
typedef enum {
        OS_NOTIFY_NO_ACTION = 0,                                /**< subject task receives event, but its notification value is not updated */
        OS_NOTIFY_SET_BITS = 1,                                  /**< notification value of subject task will be bitwise ORed with task value */
        OS_NOTIFY_INCREMENT = 2,                               /**< notification value of subject task will be incremented by one */
        OS_NOTIFY_VAL_WITH_OVERWRITE = 3,          /**< notification value of subject task is unconditionally set to task value */
        OS_NOTIFY_VAL_WITHOUT_OVERWRITE = 4,    /**< if subject task has a notification pending then notification value
 ...
 *
 */
#define OS_TASK_RESUME_FROM_ISR(task) vTaskResumeFromISR(task)
 ...
 *
 */
#define OS_EVENT_CREATE(event) do { event = pdFALSE; } while(0)
 ...
 *
 */
#define OS_EVENT_DELETE(event) do { event = pdFALSE; } while(0)
 ...
 *
 */
#define OS_EVENT_SIGNAL(event) do { event = pdFALSE; } while(0)
 ...
 *
 */
#define OS_EVENT_SIGNAL_FROM_ISR(event) do { event = pdFALSE; } while(0)
/**
 ...
 *
 */
#define OS_EVENT_WAIT(event, timeout, result) do { if(event == pdFALSE){event = pdTRUE;\
								result = pdTRUE;\
								}else{\
									result = pdFALSE;\
								}\
 ...
 *
 */
#define OS_ENTER_CRITICAL_SECTION() //CRITICAL_REGION_ENTER()
 ...
 *
 */ 
#define OS_LEAVE_CRITICAL_SECTION() //CRITICAL_REGION_EXIT()
