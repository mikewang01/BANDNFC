...
void ble_wechat_on_ble_evt(ble_wechat_t *p_wcs,ble_evt_t * p_ble_evt, data_handler *p_data_handler) {
		switch (p_ble_evt->header.evt_id)
    {
        ...
        case BLE_GATTS_EVT_WRITE:
						 Y_SPRINTF("[wechat]:BLE_GATTS_EVT_WRITE recieved");
            on_write(p_wcs,p_ble_evt,p_data_handler);
            break;
        case BLE_GATTS_EVT_HVC:
					  Y_SPRINTF("[wechat]:BLE_GATTS_EVT_HVC recieved");
...
uint32_t ble_wechat_add_service(ble_wechat_t *p_wechat)
{
		...
		//uint8_t wechat_uuid_type = BLE_UUID_TYPE_UNKNOWN;
    //err_code = sd_ble_uuid_vs_add(&base_uuid, &(wechat_uuid_type));
...
//get WeChat Service handle
uint16_t ble_wechat_get_service_handle(ble_wechat_t *p_wechat)
{
		return p_wechat->service_handle;
}
