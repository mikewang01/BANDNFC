...
void TRACKING_enter_low_power_mode()
{
	...
	// If OLED display is off, we should set Touch IC to a low power mode
	if (UI_is_idle()) {
...
void TRACKING_exit_low_power_mode(BOOLEAN b_force)
{
	...
	cling.hr.b_perform_wearing_detection = TRUE;
	
	Y_SPRINTF("[TRACKING] need perform wearing detection ");
}
...
void TRACKING_algorithms_proc(ACCELEROMETER_3D A)
{
	...
	if (act_motion == MOTION_STATIONARY) {
		...
		if (act_motion == MOTION_WALKING) {
			...
			cling.activity.day.distance += 12; // 0.75 meters
			...
			cling.activity.day.distance += 23; // 1.41 meters
			cling.activity.step_detect_ts = CLK_get_system_time();
			SLEEP_wake_up_upon_motion();
			BATT_charging_update_steps(1);
		}
...
{
	I32U t_curr = CLK_get_system_time();
	t_curr -= cling.activity.step_detect_ts;
...
//#ifdef _SLEEP_SIMULATION_
	N_SPRINTF("MINUTE UPDATE: %d, %d, %d, %d, %d, %d, %d, %d, %d",
		pminute->epoch, pminute->sleep_state, pminute->walking, pminute->running, pminute->distance, pminute->calories, pminute->skin_touch_pads, pminute->skin_temperature, pminute->heart_rate);
//#endif
//#endif
...
void TRACKING_data_logging()
{
	...
	if (cling.time.local_day_updated) {
		...
		cling.activity.sleep_36_hr = cling.activity.sleep_by_noon;
	}
		...
		switch (mode) {
			...
			case TRACKING_CALORIES:
				*value = (t->day.calories >> 4);
				break;
			case TRACKING_SLEEP:
				*value = t->sleep_36_hr; //t->day.sleep;
				break;
