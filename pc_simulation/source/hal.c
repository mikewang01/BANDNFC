...
//    ble_debug_assert_handler(error_code, line_num, p_file_name);
    ...
    Y_SPRINTF((const char *)p_file_name);
    N_SPRINTF("\r\n");
...
static void _bond_manager_error_handler(uint32_t nrf_error)
{
	...
	if (BTLE_is_connected()) {
			Y_SPRINTF("[HAL] Bond manager error - BLE disconnect");
			// Disconnect BLE service
			BTLE_disconnect();
	}
...
static void _touch_ic_binary_program()
{
#ifndef _CLING_PC_SIMULATION_
#ifdef _TOUCH_IC_PROGRAM_
	if (cling.whoami.touch_ver != BUILD_MINOR_TOUCH_RELEASE_NUMBER) {
		cling.whoami.hssp = HSSP_program_main();
		
		nrf_delay_ms(3);
		
		GPIO_init(); // Reinitialize all the GPIO pins
	} else {
		cling.whoami.hssp = 0xff;
	}
#else
	cling.whoami.hssp = 0xff;
#endif
...
void HAL_init(void)
{
  ...
  LT1PH03_Init();
	...
	// Touch IC programming
	_touch_ic_binary_program();
#endif
